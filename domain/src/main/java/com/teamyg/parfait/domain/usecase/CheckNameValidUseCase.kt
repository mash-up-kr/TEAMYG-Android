 ...
 *
 * 규칙은 서버 정규식 `^[가-힣A-Za-z0-9]+(?: [가-힣A-Za-z0-9]+)*$` 와 같은 집합으로 유지한다 —
 * 서버보다 느슨하면 여기를 통과한 이름이 서버에서만 튕겨 필드에 아무 표시도 남지 않는다.
 */
...
private enum class NameValidation(
     ...
     * 자모·일본어·아랍 숫자·non-breaking space 가 통과한 뒤 서버에서만 400 으로 튕긴다.
     */
    CheckValidCharacter(
        isValid = { name ->
            name.all { char ->
