 ...
 */
@Stable
class BatchRevealState internal constructor(
    private val settled: () -> List<Boolean>,
) : RevealState {
    private var ready by mutableStateOf(false)

    val shown: Boolean get() = isBatchShown(settled = settled(), ready = ready)

    override fun isRevealed(index: Int): Boolean = shown
...
/**
 * **빈 목록은 완료가 아니다** — 조회가 오기 전에는 기다릴 대상 자체가 없는데, 그 순간을
 * 완료로 세면 빗장이 먼저 풀려 뒤늦게 온 것들이 하나씩 뜬다.
 */
fun isBatchReady(settled: List<Boolean>): Boolean = settled.isNotEmpty() && settled.all { it }

/**
 * 한 번 [ready] 가 된 뒤에는 다시 가리지 않는다 — 항목이 하나 추가될 때마다 화면 전체가
 * 사라졌다 나타나면 더 거슬린다.
 */
fun isBatchShown(
    settled: List<Boolean>,
