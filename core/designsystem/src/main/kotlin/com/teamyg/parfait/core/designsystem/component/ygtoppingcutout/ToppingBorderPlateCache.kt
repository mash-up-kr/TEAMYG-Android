...
internal object ToppingBorderPlateCache {
    ...
    private val entries = object : LinkedHashMap<PlateKey, ToppingBorderPlate>(MAX_ENTRIES, LOAD_FACTOR, true) {
        override fun removeEldestEntry(eldest: Map.Entry<PlateKey, ToppingBorderPlate>) = size > MAX_ENTRIES
    }
    ...
    fun get(
        ...
        outsetPx: Float,
        aspectRatio: Float,
    ): ToppingBorderPlate? = synchronized(entries) { entries[PlateKey(outline, outsetPx, aspectRatio)] }
    ...
    fun put(
        ...
        synchronized(entries) { entries[PlateKey(outline, outsetPx, aspectRatio)] = plate }
    }
