...
### 2.2.1 frontmatter 파싱은 개행을 넘지 않아야 한다

`^{key}:\s*(.+)$` 로 필드를 뽑으면 `\s*` 가 개행을 넘어가 두 가지가 조용히 깨진다.

- **블록 리스트가 첫 항목만 잡힌다.** `related_code:` 다음 줄부터 `- item` 이 오는 형태에서
  `(.+)$` 가 첫 `- item` 에 앉고 나머지는 사라진다.
- **값이 빈 필드가 다음 필드의 값을 삼킨다.** `related_adr:` 가 비어 있으면 그 다음 줄인
  `related_spec: login-debug-mode` 를 값으로 가져간다. 이쪽이 더 나쁘다 — 없는 값이 아니라
  **틀린 값**이 점수에 들어간다.

실측: frontmatter 가 있는 대상 문서 210개 **전부** 최소 한 필드가 잘못 파싱됐다.
다음 필드를 삼킨 경우 75건, 블록 리스트가 잘린 경우 99건.

같은 줄 공백만 허용하고(`[ \t]*`) 들여쓰기된 이어지는 줄만 값에 포함한다. 블록 리스트는
`-` 를 떼고 공백으로 이어 붙여 한 줄로 만든다 — 토크나이저가 어차피 단어로 쪼개므로
리스트 구조를 보존할 이유가 없다.
...
### 4.1 깨져 보이지만 깨지지 않은 링크 11건

두 문서에 `](conventions.md)`·`](api/README.md)`·`](parfait/specs/…)` 처럼 이 저장소에서
resolve 되지 않는 링크가 11건 있다. 처음에는 원본에 이미 깨져 있는 링크로 보고 사본에서
고치기로 했는데, **그 판단이 틀렸다.**

11건은 전부 **코드 펜스 안**이다. 옛 계획 문서가 "index.md 에 이 줄을 추가하라"는 예시를
펜스로 보여 주는 대목이고, 그 예시는 당시 `parfait/api/` 기준으로 쓰여 있다. 링크가 아니라
링크의 그림이다.

`check_links.py` 는 펜스를 비운 뒤 검사하므로(`blank_out`) 이 11건을 애초에 보지 않는다.
게이트는 그것들 없이도 exit 0 이다. 이관 도구 역시 펜스 밖만 재작성하므로 손대지 않는다.

따라서 **고치지 않는다.** 고치면 두 가지가 망가진다.

- 역사 문서의 예시가 당시 사실과 달라진다. 그 펜스는 과거에 실행된 절차의 기록이다.
- 표시 텍스트와 target 이 갈린다. 실제로 그렇게 됐다 — 라벨은 `parfait/specs/…` 를
  그대로 두고 target 만 `../specs/…` 로 바뀌어, 읽는 사람에게 표시되는 경로가 거짓이 됐다.
