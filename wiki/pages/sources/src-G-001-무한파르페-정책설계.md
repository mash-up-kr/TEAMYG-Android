...
updated: 2026-09-17
status: superseded
superseded_by: [src-G-001-무한파르페-정책설계-v0.2]
---

# 무한 파르페 정책 설계 (G-001)

> 이 판본은 [[src-G-001-무한파르페-정책설계-v0.2]]로 **전면 대체됐다.** `v0.2`가
> 배치를 지그재그로 바꾸고 좌표·규격·라벨까지 한 문서에 다시 모았다. 현행 사실은
> `v0.2`에서 확인한다. 아래는 이 판본이 남긴 것과 `v0.2`가 지운 것이다.
...
그리드 배치 규칙, 요소별 z-index, 라벨 표기 기준까지. **기능정의서와는 서로
대체하지 않는다** — 같은 계열 안에서만 판본이 올라간다.
...
- **그룹 목록은 파르페 잔 하나다.** 그룹이 늘수록 템플릿이 아래로 무한히
  ...
  쌓인다. **이 패턴은 폐기됐다** — [[src-G-001-무한파르페-간격-정책-v0.2]]가
  지그재그로 재설계했고 `v0.2`가 그것을 확정했다. 구조는 [[infinite-parfait]]에
  있다.
- **정렬은 활동순이다.** 투표에서 9명이 활동순을 골랐고 가입순·이름순에는 표가
  없다. 아래 "결정과 그 근거"를 본다. **`v0.2`가 "활동순 확정"이라는 문장을
  붙였다** — 이 판본에는 투표 결과만 있었다.
...
## v0.2가 이 판본의 미결을 어떻게 처리했나

| 이 판본의 미결 | `v0.2` |
|---|---|
| 토핑 이미지 뷰어 정책 ("다음에.. 누군가 정해야 함") | **항목 삭제 확정 (불필요)** |
| 화면 재진입 시 자동 재조회 ("정의에 따름") | **진행 (확정)** |
| 2열·1열의 이름 | 패턴 자체가 폐기돼 **물음이 사라졌다** |
| 크림 가로 길이 1안 대 2안 | **답 없이 없어졌다** |
| `N = 0`의 빈 상태 안내 (크림 상단 **42% 지점**) | 온보딩 툴팁 `G-001-Empty`로 옮기고 **폐기 명시**(간격 `v0.3`) |
| 체리 `TBD` | 그대로 `TBD` |
| 그룹 데이터 캐싱 여부 | 그대로 없다 |

**같은 문서 안에서 두 방식이 섞였다.** 이미지 뷰어는 "삭제 확정"이라 적고
지웠는데 크림 가로 길이는 말없이 빠졌다. 뒤엣것은 해소로 읽지 않는다
([[purpose]]).
...
- 그룹 데이터 캐싱 여부는 **취소선으로 지워졌다.** 답이 적힌 것이 아니다
  ([[purpose]]).
- 크림 가로 길이 1안·2안이 `v0.2`에서 답 없이 사라졌다.
