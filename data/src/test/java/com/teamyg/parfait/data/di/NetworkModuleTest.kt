...
class NetworkModuleTest {
    ...
    fun unauthenticatedOkHttpClient_hasItsOwnDispatcher() {
        // Given 메인 클라이언트와 자격증명을 붙이지 않는 클라이언트
        ...
        assertNotSame(mainClient.dispatcher, unauthenticatedClient.dispatcher)
    }

    @Test
    fun unauthenticatedOkHttpClient_doesNotCarryAuthenticator() {
        // Given 메인 클라이언트에는 인증기가 달려 있다
        assertSame(tokenAuthenticator, mainClient.authenticator)

        // When 자격증명 없는 클라이언트의 인증기를 본다
        // Then 달려 있지 않다 — 재발급 자신의 401 이 인증기를 재진입시키면 안 된다
        assertNotSame(tokenAuthenticator, unauthenticatedClient.authenticator)
    }

    @Test
    fun unauthenticatedOkHttpClient_doesNotCarryAuthInterceptor() {
