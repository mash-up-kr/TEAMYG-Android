...
interface ParfaitImageRemoteDataSource {
     ...
     * 대조하지 않는다(`docs/api/parfait-image.md`). 같은 이미지를 두 번 배치할 수 없다.
     */
     ...
     * 실패 코드와 검사 순서는 `docs/api/parfait-image.md` 참고.
     */
     ...
     * (`docs/api/parfait-image.md`). 멱등이 아니라 같은 배치를 두 번 지우면 404 다.
     */
