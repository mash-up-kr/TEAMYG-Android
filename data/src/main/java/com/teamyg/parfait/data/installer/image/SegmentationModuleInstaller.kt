...
/**
 * 모듈 준비의 단일 소유자. 코루틴 스코프를 들지 않는다 — 대기는 호출자의 스코프에서 일어나고,
 * 진행 중인 설치만 여기서 공유한다.
 */
@Singleton
...
constructor(
    ...
    private var inFlight: Deferred<ModuleInstallSignal>? = null
    ...
    suspend fun ensureInstalled(): ModuleInstallOutcome {
        ...
        val pending = mutex.withLock {
            ...
            inFlight?.takeIf { !it.isCompleted } ?: gateway.install().also { inFlight = it }
        }
    ...
    private suspend fun timedOut(pending: Deferred<ModuleInstallSignal>): ModuleInstallOutcome {
